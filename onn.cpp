...
void add_vertex(Vertex *temp)   //add new polygon vertex using dll
{
    ...
    {
        head = temp; head->next = head->prev = temp;
    }
...
bool intersect(Point a, Point b, Point c, Point d)
{
    ...
    // using xor property
    else if((((int)to_left(a,b,c))^((int)to_left(a,b,d))) && (((int)to_left(c,d,a)) ^ ((int)to_left(c,d,b))))return true;
...
// initialise ear vertices which will be updated as we add diagonals
void ears( void )
{
    //cout<<"Hi\n"; //debug
...
{
    int n;  
    cout<<"Enter number of vertices - ";cin>>n; //input number of vertices
    ...
    if(!diag_count)cout<<"No diagonal could be added\n";
